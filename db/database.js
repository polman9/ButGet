  ...
  db.transaction(tx => {
    categories.forEach(cat => {
      tx.executeSql(
        ...
        (_, error) => console.error(`Error inserting category ${cat.name}`, error)
      );
    });
  });
  ...
  db.transaction(tx => {
    subcategories.forEach(subcat => {
      tx.executeSql(
        ...
        () => console.log(`Subcategory ${subcat.name} for category ${subcat.category} inserted`),
        (_, error) => console.error(`Error inserting subcategory ${subcat.name}`, error)
      );
    });
  });
};
